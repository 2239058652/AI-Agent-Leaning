...
1. 防绕过：服务端保存待确认操作，前端只携带一次性 `confirmationId`，确认后单次消费。
2. 参数校验：生成待确认操作前完成白名单、敏感标记和参数 Schema 校验。
3. 错误语义：参数错误返回明确失败；前端检查 HTTP 状态和业务 `success`，禁止显示假成功。
...
**下一动作**：完成进阶段7前的 3 项确认闭环修复，再做总验收并进入阶段7。
