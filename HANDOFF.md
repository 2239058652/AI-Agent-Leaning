...
| **当前主线** | **L3 巩固收尾**：M1–M5 已过关，先修敏感确认的 3 个闭环问题，再进阶段7 |
...
- 敏感确认可被直接调用绕过弹窗：需改为后端保存待确认操作并签发一次性 `confirmationId`
- 敏感工具在生成确认前未完成参数校验，确认执行也未走 `ToolValidator`
- 确认接口错误语义不一致，前端未检查 HTTP/业务成功状态，可能显示假成功
